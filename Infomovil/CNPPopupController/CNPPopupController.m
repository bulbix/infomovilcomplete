...
- (instancetype)initWithTitle:(NSAttributedString *)popupTitle
    ...
    if (self) {
        ...
        self.inputText = [[UITextField alloc] initWithFrame:CGRectMake(10, 100, 500, 30)];
        [self.inputText setBackgroundColor:[UIColor whiteColor]];
        self.inputText.layer.cornerRadius = 5.0f;
    }
    return self;
}
...
- (void)setUpPopup {
    ...
    self.maskView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 320, screenRect.size.height)];
...
- (void)actionButtonPressed:(CNPPopupButton *)sender {
    ...
    [self dismissPopupControllerAnimated:YES withButtonTag:sender.item.tag];
}
...
- (void)dismissPopupControllerAnimated:(BOOL)flag {
    [self dismissPopupControllerAnimated:flag withButtonTag:0];
}

//- (void)dismissPopupControllerAnimated:(BOOL)flag withButtonTitle:(NSString *)title
- (void)dismissPopupControllerAnimated:(BOOL)flag withButtonTag:(NSInteger)bTag {
...
//                         }
                         if ([self.delegate respondsToSelector:@selector(popupController:didDismissWithButtonTag:)]) {
                             [self.delegate popupController:self didDismissWithButtonTag:bTag];
                         }
...
//    [button setBackgroundColor:item.backgroundColor];
    [button setBackgroundImage:[UIImage imageNamed:@"btnazulnuevo"] forState:UIControlStateNormal];
